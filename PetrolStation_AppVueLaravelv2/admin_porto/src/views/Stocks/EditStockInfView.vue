<template>
    <DefaultLayout>
      ...
      <div class="grid w-full grid-cols-1 gap-9 sm:grid-cols-1">
        <div class="flex flex-col gap-9">
          <form @submit.prevent="updateStock" class="flex flex-col w-full p-5 bg-white rounded-md shadow-lg md:p-10">
  ...
  const pageTitle = ref('Edit Stock');
  ...
  const fetchStockData = async () => {
    const stockId = router.currentRoute.value.params.id;
    try {
      const response = await axios.get(`/api/stocks/${stockId}`);
      const stockData = response.data;
      formData.value = { ...stockData };
    } catch (error) {
  ...
  const updateStock = async () => {
    const stockId = router.currentRoute.value.params.id;
    try {
      await axios.put(`http://127.0.0.1:8000/api/stocks/${stockId}`, formData.value);
      router.push('/stocks');
    } catch (error) {
