    ...
    <DefaultLayout>
      ...
      <div class="grid w-full grid-cols-1  gap-9 sm:grid-cols-1"> <!-- Changed grid-cols-2 to grid-cols-1 for small screens -->
        <div class="flex flex-col gap-9">
          ...
          <form action="" method="post" @submit.prevent="submitForm" class="flex flex-col w-full p-5 bg-white rounded-md shadow-lg md:p-10 " style="width: 900px;">
            <label for="tank_number" class="mb-5">
              <span>Tank Number</span>
              <input
                type="text"
                name="tank_number"
                v-model="formData.tank_number"
                id="tank_number"
            ...
            <!-- Update the attribute names accordingly -->
            <label for="amount" class="mb-5">
              <span>Amount Sold</span>
              <input
                type="text"
                name="amount" v-model="formData.amount"
                id="amount"
                class="w-full p-3 mt-2 border border-gray-300 rounded shadow outline-none appearance-none bg-inherit shadow-gray-100 text-neutral-800"
                placeholder=" "
                required
            ...
            <label for="volumeLevel" class="mb-5">
              ...
              <input
                type="text"
                name="volumeLevel" v-model="formData.volumeLevel"
                id="volumeLevel"
                class="w-full p-3 mt-2 border border-gray-300 rounded shadow outline-none appearance-none bg-inherit shadow-gray-100 text-neutral-800"
                placeholder=" "
                required
...
const formData = ref<{
  ...
  amount: string;
  category: string;
  volumeLevel: string; // Added volumeLevel attribute
  description: string;
}>({
  tank_number: '',
  amount: '',
  category: '',
  volumeLevel: '', // Initialized volumeLevel
  description: ''
});
...
const submitForm = async () => {
  try {
    const response = await axios.post('/api/fuels', formData.value) // Updated endpoint to /api/fuels
