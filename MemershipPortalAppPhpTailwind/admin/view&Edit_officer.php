...
$region =$_POST["region"];
 ...
 $update_role = "UPDATE users SET  name='$name',email='$email',role='$role',region='$region'
    WHERE  id='$id'";
    ...
    if (mysqli_query($conn, $update_role)) {
        ...
        echo "Error: " . $update_role. "<br>" . mysqli_error($conn);
         header("location:view&Edit_role.php");
           exit();
    }
}
?>
...
<div class="p-4 sm:ml-64">
   <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
    ...
    <form method="POST" action="">
     <?php
                $select_role = "select * from users where user_id = '" .$_GET['user_id']. "'" or die(mysqli_error($conn));
  ...
  <div class="-mx-3 md:flex mb-6">
    <div class="md:w-1/2 px-3 mb-6 md:mb-0">
      <label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2" for="grid-first-name">
        Assign officer to region respectively
      </label>
      <select type="text" name="region" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
    ...
    <div class="md:w-1/2 px-3">
      ...
      <select type="text" name="role" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
          ...
          <option value="region">Region officer</option>
          <option value="Not-officer">Pending</option>
        </select>
    </div>
  </div>
...
<hr>
<br>
   <button type="submit" name="update" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" style="margin-left:700px">Update</button>
